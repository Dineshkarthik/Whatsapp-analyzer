...
def index():
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            response["the_silent_killer"] = (
                messages["sender"].value_counts().idxmin()
            )
            media = messages[(messages.message == " <Media omitted>")]
            if len(media):
                response["media_count"] = (
                    media["sender"].value_counts().to_dict()
                )
                response["media_share_freak"] = (
                    media["sender"].value_counts().idxmax()
                )
            else:
                response["media_count"] = {}
                response["media_share_freak"] = ""
            response["date_chart"] = messages["date"].value_counts().to_dict()
            response["most_active_date"] = (
                messages["date"].value_counts().idxmax()
            )
            ...
            response["avg_no_of_msgs_per_day"] = (
                messages["date"].count() / messages["date"].nunique()
            )
